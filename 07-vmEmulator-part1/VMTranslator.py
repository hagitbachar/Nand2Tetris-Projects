...
def check_command_type(command_type):
    return {
        'push': 'C_PUSH',
        'pop': 'C_POP',
        'add': 'C_ARITHMETIC',
...
def parse_commande(command):
    ...
    command_type = check_command_type(split_command)
    arg_first = 'null'
    arg_second = 'null'
    if command_type == 'C_PUSH' or command_type == 'C_POP':
        arg_first = command.split(' ')[1]
        arg_second = command.split(' ')[2]
    yield split_command, arg_first, arg_second
...
if '.vm' not in vm_file_name: # that's mean it's a directory
    ...
    file_or_dir_name = os.path.join(vm_file_name, os.path.basename(vm_file_name)) + '.asm'
